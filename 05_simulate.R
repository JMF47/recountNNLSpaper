...
txs_sim=import('/dcl01/leek/data/ta_poc/GencodeV25/gencodeV25.sim.gtf')
...
simAbundance = function(gene, sim_info){
	...
	abundance = gtools::rdirichlet(1, rep(1/k, k))
	return(abundance)
}

for(pe in c(FALSE, TRUE)){
	for(rl in c(37, 50, 75, 100, 150)){
		message(rl)
		##########################################################################################
		###  Determine the simulation truth
		##########################################################################################
		set.seed(rl+1) 
		...
		### Quantify
		##########################################################################################
		paired = pe*1+1 # Convert pe variable to FALSE=1, TRUE=2
		...
		if(paired==1){
			...
			dir.create(rail_dir)
			write.table(man, sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE, 
				file=paste0(rail_dir, "/manifest.txt"))	
		}
		...
		outdir = paste0("/dcl01/leek/data/ta_poc/geuvadis/simulation/", condition)
		setwd(outdir)
		load("reads/sim_counts_matrix.rda") ### Automatically created from ballgown
		...
		recountNNLS = assays(rse_tx)$fragments[match(truth$tx_name, rownames(rse_tx)),]
